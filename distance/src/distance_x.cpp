...
void callbackTwist (const geometry_msgs::Twist::ConstPtr&msg) {
    ...
    // 원래는 firstPushed 비교하면 되지만  //그냥 한번 포인터 연습해 봤음 --- 그냥 전역변수로 선언했으니깐 
    // 의미 없을 수도 있지만 
    int *firstPushedPtr = &firstPushed;
    
    // 키가 안 눌리고 x가 0이 아니면 시작시간 기록
    if (*(firstPushedPtr) == 0 && msg->linear.x != 0) {
        // 시간 넣어주기 및 초기화 
        startTime = ros::Time::now();
        *(firstPushedPtr) = 1;  // 처음 눌렸으므로 1 값을 준다
    ...
    } else if ((*firstPushedPtr) == 1 && msg->linear.x == 0) {
        // 마지막 시간 저장
        endTime = ros::Time::now();
        *(firstPushedPtr) = 0; // 멈춤이 눌리면 0으로 초기화
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "sub_distance");
    ros::NodeHandle nh;
